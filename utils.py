...
def conv_layer_2d_special_stride(x, filter_shape, stride, trainable=True):
    W = tf.get_variable(
        name='weight',
        shape=filter_shape,
        dtype=tf.float32,
...
def deconv_layer_2d(x, filter_shape, output_shape, stride, trainable=True):
    x = tf.pad(x, [[0,0], [filter_shape[0],filter_shape[0]], [filter_shape[1],filter_shape[1]], [0,0]], mode='reflect')
    W = tf.get_variable(
        name='weight',
        shape=filter_shape,
        dtype=tf.float32,
    ...
    return x[:, filter_shape[0]:-filter_shape[0], filter_shape[1]:-filter_shape[1], :]
...
def plot_SR_data(idx, LR, SR, path,fn):
    #fn added by gbh
    for i in range(LR.shape[0]):
        ...
        plt.savefig(path+'/'+fn+'_img{0:05d}.png'.format(idx[i]), dpi=200, bbox_inches='tight')
        plt.close()
...
def generate_TFRecords(filename, data, ro=None, mode='test', K=None):
    '''
    ...
    '''
    if (mode == 'train') | (mode=='valid'):
        assert K is not None, 'In training mode, downscaling factor K must be specified'
        data_LR = downsample_image(data, K)
        data_HR=downsample_image(data, 1)#sess.run(tf.convert_to_tensor(data))
        with tf.Session() as sess:
            ro=sess.run(tf.convert_to_tensor(ro))
 
    elif (mode == 'test') & (K is not None) :
        data_LR=downsample_image(data,K)
    ...
    with tf.python_io.TFRecordWriter(filename) as writer:
        if (mode=="train") | (mode=='valid'):
            for j in range(data_LR.shape[0]):
                ...
                h_HR, w_HR, _ = data_HR[j, ...].shape
                features = tf.train.Features(feature={
                                     'index': _int64_feature(j),
                                        'ro': _bytes_feature(ro.tostring()),
            ...
            for j in range(data_LR.shape[0]):
                h, w, c = data_LR[j, ...].shape

                features = tf.train.Features(feature={
                                     'index': _int64_feature(j),
                                   'data_LR': _bytes_feature(data_LR[j, ...].tostring()),
                                      'h_LR': _int64_feature(h),
                                      'w_LR': _int64_feature(w),
                                      'c': _int64_feature(c)})
                example = tf.train.Example(features=features)
                writer.write(example.SerializeToString())
